...
const INIT_MEMBERS: DemoMember[] = [
  { id:1, name:"Mama", avatar:"👩", photo:null, color:T.red,   role:"Elternteil" },
  { id:2, name:"Papa", avatar:"👨", photo:null, color:T.blue,  role:"Elternteil" },
  { id:3, name:"Lena", avatar:"👧", photo:null, color:T.green, role:"Kind" },
  { id:4, name:"Max",  avatar:"👦", photo:null, color:T.amber, role:"Kind" },
];
...
function FamilyApp() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setMembers((prev) => {
        ...
        const matched = prev.map((m) => {
          const row =
            (m.supabaseId && rows.find((r) => r.id === m.supabaseId)) ||
            rows.find((r) => r.name === m.name) ||
            null;
          if (row) used.add(row.id);
          return { m, row };
        });
        const pool = rows.filter((r) => !used.has(r.id));
        let pi = 0;
        return matched.map(({ m, row }) => {
          const r = row ?? pool[pi++];
          if (!r) return m;
          return {
            ...m,
            ...
            name: r.name,
            avatar: r.avatar ?? m.avatar,
            color: r.color ?? m.color,
            role: r.role ?? m.role,
          };
        });
      });
  ...
  const uploadPhoto = async (memberId: number, file: File) => {
    try {
      ...
      if (!member) return;
      const rowId = member.supabaseId;
      if (!rowId) {
        alert("Mitglied ist noch nicht mit Supabase verknüpft. Bitte Seite neu laden.");
        return;
      }
